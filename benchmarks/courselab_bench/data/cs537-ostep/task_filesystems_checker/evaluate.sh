...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
    ...
    rm -f filesystemschecker *.o 2>/dev/null || true

    echo "Building filesystemschecker"
    if [ -f Makefile ]; then
        ...
        if timeout 300 gcc -D_GNU_SOURCE -std=gnu11 -Wall -Werror -O2 -o filesystemschecker *.c; then
            BUILD_SUCCESS=1
        else
            BUILD_SUCCESS=0
        fi
