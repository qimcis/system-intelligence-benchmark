...
cd /workspace/ostep-projects/initial-utilities/wzip
...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
    ...
    if [ -f Makefile ]; then
        ...
        if timeout 300 gcc -D_GNU_SOURCE -std=gnu11 -Wall -Werror -O2 -o wzip *.c; then
            BUILD_SUCCESS=1
        else
            BUILD_SUCCESS=0
        fi
    ...
    echo "Running tests"
    if timeout 600 bash test-wzip.sh 2>&1 | tee test_output.txt; then
